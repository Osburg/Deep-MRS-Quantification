...
class Engine:
    def __init__(self, parameters):
        ...
        self.saving_dir = (
            parameters["parent_root"] + parameters["child_root"] + parameters["version"]
        )
        ...
        if self.MM:
            if parameters["MM_model"] == "lorntz":
                ...
                # self.MM_d = (np.pi ** self.MM_d) * ((self.MM_d) ** 2) / (2 * np.log(2)) # noqa: E501
    ...
    def getSignals(self, min_c, max_c, f, d, ph, noiseLevel, ns, mm_cond):
        ...
        ampl = min_c + np.multiply(
            np.random.random(size=(ns, numOfSig)), (max_c - min_c)
        )
        ...
        noise = np.random.normal(
            0, noiseLevel, (ns, self.sigLen)
        ) + 1j * np.random.normal(0, noiseLevel, (ns, self.sigLen))
        signal = signal + noise
    ...
    def getSignal(self, ampl, shift, alpha, ph, noiseLevel, mm_cond):
        ...
        signal = np.matmul(ampl[:, 0:numOfSig], basisset[0 : self.sigLen, :].T)
        noise = np.random.normal(
            0, noiseLevel, (1, self.sigLen)
        ) + 1j * np.random.normal(0, noiseLevel, (1, self.sigLen))
        y = np.multiply(signal, np.exp(freq * 1j).T)
        y = y.T * np.exp(ph * 1j)
        y = np.multiply(y, np.exp(-alpha * self.t))
        return y + noise.T, y
    ...
    def get_augment(self, signals, n, f_band, ph_band, ampl_band, d_band, noiseLevel):
        """
        It takes a signal, and creates a number of augmented signals by adding noise,
        shifting the frequency, and shifting the phase
        ...
        :param noiseLevel: the standard deviation of the noise
        :return: the augmented signal, the amplitude, the delay, the shift and the
        phase.
        """
        list_ = []
        list_.append(signals)
        ...
        for i in range(0, lens):
            ...
            freq = (
                -2
                * math.pi
                * (shift_t[i * numOfSamplei : (i + 1) * numOfSamplei])
                * self.t
            )
            ...
            noise = np.random.normal(
                0, noiseLevel, (len(signal), numOfSamplei)
            ) + 1j * np.random.normal(0, noiseLevel, (len(signal), numOfSamplei))
            y_i = y + noise
            list_.append(y_i)
        y = np.hstack(list_)
    ...
    def savefig(self, path, plt_tight=True):
        """
        It saves the current figure to the specified path, with the specified format,
        and with the specified dpi

        :param path: the name of the file to save
        :param plt_tight: If True, it will make the plot look nice, defaults to True
        (optional)
        """
        # plt.ioff()
        if plt_tight:
    ...
    def loadModel(autoencoder, path):
        ...
        return autoencoder.load_state_dict(
    ...
    def cal_snr(self, data, endpoints=128, offset=0):
        ...
        the last 128 points of the data

        :param data: the data you want to calculate the SNR for
        :param endpoints: The number of points at the beginning and end of the data to
        use for calculating the noise, defaults to 128 (optional)
        :param offset: The number of points to skip at the beginning of the data,
    ...
    def cal_snrf(self, data_f, endpoints=128, offset=0):
        """
        It takes the absolute value of the maximum value of the data, and divides it by
        the standard deviation of the data

        :param data_f: the data in the frequency domain
        :param endpoints: the number of points to use for the standard deviation
        calculation, defaults to 128 (optional)
        :param offset: The offset of the data to be used for calculating the SNR,
        defaults to 0 (optional)
        :return: The max of the absolute value of the data_f divided by the standard
        deviation of the real part of the data_f.
        """
    ...
    def fillppm(self, y1, y2, ppm1, ppm2, rev, alpha=0.1, color="red"):
        p1 = int(self.ppm2p(ppm1, len(y1)))
        p2 = int(self.ppm2p(ppm2, len(y1)))
        n = p2 - p1
        x = np.linspace(int(ppm1), int(ppm2), abs(n))
        plt.fill_between(
            np.flip(x), y1[p2:p1, 0].real, y2[p2:p1, 0].real, alpha=0.1, color="red"
        )
        if rev:
    ...
    def plotsppm(self, sig, ppm1, ppm2, rev, linewidth=0.3, linestyle="-", color=None):
        ...
        # g = plt.plot(np.flip(x), sig[p2:p1,:].real, linewidth=linewidth, linestyle=linestyle, color=color) # noqa: E501
        if rev:
            plt.gca().invert_xaxis()

    def normalize(self, inp):
        """
        It takes the absolute value of the input, divides it by the maximum absolute
        value of the input, and then multiplies
        it by the exponential of the angle of the input

        :param inp: The input signal
        :return: The magnitude of the input divided by the maximum magnitude of the
        input, multiplied by the complex exponential of the angle of the input.
        """
    ...
    def plot_basis2(self, basisset, ampl):
        ...
        for i in range(0, len(basisset.T) - 1):
            self.plotppm(
                -1 * i + fft.fftshift(fft.fft(ampl * basisset[:, i])), 0, 10, False
            )
        self.plotppm(
            -20 * (i + 1) + fft.fftshift(fft.fft(basisset[:, i + 1])), 0, 10, True
        )
        ...
        for i in range(0, len(self.basisset.T)):
            vv = fft.fftshift(
                fft.fft(
                    ampl[0, i]
                    * self.basisset[:, i]
                    * np.exp(-2 * np.pi * 1j * fr * self.t.T)
                    * np.exp(-1 * damp * self.t.T)
                )
            )
            ax = self.plotppm(-2.2 * (i + 2) + vv.T, 0, 5, False)
            sns.despine(left=True, right=True, top=True)
            plt.text(0.1, -2.2 * (i + 2), self.met_name[i], fontsize=8)
            ax.tick_params(left=False)
            ax.set(yticklabels=[])

    def Lornz(self, ampl, f, d, ph, Crfr, Crd):
        return ampl * np.multiply(
            np.multiply(
                np.exp(ph * 1j), np.exp(-2 * math.pi * ((f + Crfr)) * self.t.T * 1j)
            ),
    ...
    def data_proc(self):
        ...
        The training set is the first part of the dataset, and the test set is the last
        part of the dataset.
        ...
        if self.quality_filt[0]:
            cond = np.mean(
                np.abs(y_f[self.quality_filt[1] : self.quality_filt[2], :]), axis=0
            )
            # con2 = np.mean(np.abs(y_f[self.quality_filt[3]:self.quality_filt[4],:]), axis=0) # noqa: E501
        ...
        sio.savemat(
            self.saving_dir + "test_" + str(self.test_nos) + "_testDB.mat",
        ...
        Jmrui.write(
            Jmrui.makeHeader(
                "basis_set",
                np.size(self.basisset, 0),
                np.size(self.basisset, 1),
                self.t_step * 1000,
                0,
                0,
                self.trnfreq * 1e6,
    ...
    def data_prep(self):
        ...
        The first thing it does is check if the simulation parameters are not None. If
        they are not None, then it checks if the simulation is now. If the simulation
        is now, then it gets the signals using the getSignals function. It then saves
        the data in a .npz file. If the simulation is not now, then it tries to load
        the data from the .npz file. If it can't find the .npz file, then it prints
        an error message.

        If the simulation parameters are None, then it gets the data using the
        data_proc function.

        If the augmentation parameters are not None, then it augments the data
        using the data_aug function.

        It then takes the fft of the data.

        If the pre_plot parameter is True, then it plots the histogram of the signal to
        noise
        """
        if self.sim_params is not None:
            if self.sim_now:
                data = self.getSignals(
                    self.sim_params[0],
                    self.sim_params[1],
                    self.sim_params[2],
                    self.sim_params[3],
        ...
        if self.pre_plot:
            ...
            plt.close()
            self.plotppm(
                fft.fftshift(fft.fft((y[:, 0]), axis=0)),
                0,
                5,
                True,
                linewidth=1,
    ...
    def to_tensor(self, y_norm):
        """
        It takes the normalized signal and truncates it to the length of the truncated
        signal.

        It then converts the truncated signal to a tensor.

        If the simulation parameters are not None, it splits the truncated signal into
        a training and validation set.

        If the simulation parameters are None, it splits the truncated signal into a
        ...
        if self.sim_params is None:
            ...
            my_dataset = TensorDataset(
                self.y_trun,
                torch.from_numpy(
                    np.hstack((self.ampl_t, np.expand_dims(self.alpha, 1)))
                ),
            )
            self.train, self.val = random_split(
                my_dataset,
                [
                    int((self.numOfSample) * self.tr_prc),
                    self.numOfSample - int((self.numOfSample) * self.tr_prc),
                ],
            )
    ...
    def inputSig(self, x):
        if self.in_shape == "2chan":
            return torch.cat(
                (
                    torch.unsqueeze(x[:, 0 : self.truncSigLen].real, 1),
                    torch.unsqueeze(x[:, 0 : self.truncSigLen].imag, 1),
                ),
                1,
            )
    ...
    def test_compact(self):
        ...
        The function then loads the test set of signals and passes it to the
        predict_ensembles function.

        The predict_ensembles function returns the predicted values of the metabolites,
        the frequencies, damping factors,
        ...
        if self.parameters["test_load"]:
            ...
            sio.savemat(
                self.sim_dir + "test_" + str(self.test_params[2:]) + "_testDB.mat",
                {
                    "y_test": y_test,
                    "ampl_t": ampl_t,
                    "shift_t": shift_t,
        ...
        errors_DL = pd.DataFrame(
            columns=["R2", "MAE", "MSE", "MAPE", "r2", "intercept", "coef"],
            index=self.met_name,
        )
        for i in range(0, self.ens):
            for j in range(0, self.numOfSig):
                ...
                model = LinearRegression().fit(
                    ampl_t[:, j].reshape((-1, 1)), encs[i, :, j].reshape((-1, 1))
                )
                errors_DL.iloc[j] = [
                    r2_score(ampl_t[:, j], encs[i, :, j]),
    ...
    def test(self):
        """
        The function takes in the test data and the parameters of the model and saves
        the results of the test in a folder
        """
        id = "test_" + str(self.test_params[6]) + "_" + str(self.test_params[5]) + "_"
        data = np.load(self.sim_dir + "test_" + str(self.test_params[2:]) + ".npz")
        y_test, ampl_t, shift_t, alpha_t, ph_t, snrs = [data[x] for x in data]
        ...
        net_true = pd.DataFrame(
            ampl_t[:, 0 : len(self.met_name)], columns=self.met_name
        )
        net_true["type"] = type
        ...
        lc = [
            self.met_name[i]
            for i in (np.where(self.max_c[0 : len(self.met_name)] < 0.3)[0])
        ]
        sns.set_style("whitegrid")
        sns.violinplot(
            x="variable",
            y="value",
            data=dfm[dfm["variable"].isin(lc)],
            hue="type",
            palette="Set3",
            linewidth=1,
            split=True,
        ...
        lc = [
            ...
            for i in (
                np.where(
                    (self.max_c[0 : len(self.met_name)] > 0.3)
                    & (self.max_c[0 : len(self.met_name)] < 1.01)
                )[0]
            )
        ]
        sns.violinplot(
        ...
        # test_info['p_error'] = (ph[:, 0] * 180 / np.pi) - np.expand_dims(ph_t * 180 / np.pi, axis=[1])[:, 0] # noqa: E501
        # sns.jointplot(x=test_info[['f_error','d_error']], y=test_info[['f','d']])
        ax = sns.jointplot(
            x=np.expand_dims(shift_t, axis=[1])[:, 0],
            y=fr[:, 0] - np.expand_dims(shift_t, axis=[1])[:, 0],
        )
        ...
        ax = sns.jointplot(
            x=np.expand_dims(alpha_t, axis=[1])[:, 0],
            y=damp[:, 0] - np.expand_dims(alpha_t, axis=[1])[:, 0],
        )
        ...
        ax = sns.jointplot(
            x=np.expand_dims(ph_t * 180 / np.pi, axis=[1])[:, 0],
            y=(ph[:, 0] * 180 / np.pi)
            - np.expand_dims(ph_t * 180 / np.pi, axis=[1])[:, 0],
        )
        ...
        errors_combined = pd.DataFrame(
            columns=["R2", "MAE", "MSE", "MAPE", "r2", "intercept", "coef"], index=names
        )
        idx = 0
        for id1, id2, name in zip(ids1, ids2, names):
            # var = (y_out_var[:, id1]**2 + y_out_var[:, id2]**2) + (ampl_t[:, id1]**2 + ampl_t[:, id2]**2) # noqa: E501
            # corr, _ = pearsonr(ampl_t[:, id1], ampl_t[:, id2])
            # warning! how we can calculate sd for two corrolated normal distribution!?
            # sd = 100 * np.sqrt(y_out_var[:, id1] + y_out_var[:, id2]) / (y_out[:, id1] + y_out[:, id2]) # noqa: E501
            self.calib_plot(
                ampl_t[:, id1] + ampl_t[:, id2],
                (y_out[:, id1] + y_out[:, id2]),
                None,
                None,
            )
            ...
            model = LinearRegression().fit(
                (ampl_t[:, id1] + ampl_t[:, id2]).reshape((-1, 1)),
                (y_out[:, id1] + y_out[:, id2]).reshape((-1, 1)),
            )
            errors_combined.iloc[idx] = [
                r2_score(
                    ampl_t[:, id1] + ampl_t[:, id2], (y_out[:, id1] + y_out[:, id2])
                ),
                mean_absolute_error(
                    ampl_t[:, id1] + ampl_t[:, id2], (y_out[:, id1] + y_out[:, id2])
                ),
        ...
        if self.parameters["detailed_test"]:
            ...
            for idx, name in enumerate(self.met_name):
                model = LinearRegression().fit(
                    ampl_t[:, j].reshape((-1, 1)), y_out[:, idx].reshape((-1, 1))
                )
                errors_averaged.iloc[j] = [
            ...
            for idx_null, name in enumerate(selected_met):
                ...
                self.err_plot(
                    np.expand_dims(alpha_t, axis=[1])[:, 0], err, None, "Damping", None
                )
                plt.title(name)
                self.savefig(id + "corrollation_precent" + "Damping_" + name)

                self.err_plot(
                    np.expand_dims(shift_t, axis=[1])[:, 0],
                    err,
                    None,
                    "Frequency",
                    None,
                ...
                self.err_plot(
                    np.expand_dims(snrs, axis=[1])[:, 0], err, None, "SNR", None
                )
                plt.title(name)
    ...
    def test_asig(self, shift_t, alpha_t, ph_t, nl):
        ...
        ampl_t = (
            self.min_c
            + (self.max_c - self.min_c) / 2
            + np.multiply(
                np.random.random(size=(1, 1 + self.numOfSig)), (self.max_c - self.max_c)
            )
        )
        ...
        self.plotppm(
            fft.fftshift(fft.fft((y_n[:, 0]), axis=0)),
            0,
            5,
            False,
            linewidth=1,
        ...
        self.plotppm(
            35 + (fft.fftshift(fft.fft((y_n[:, 0]), axis=0)) - np.squeeze(y_out_mean)),
            0,
            5,
            False,
            linewidth=1,
        ...
        self.plotppm(
            37.5
            + (fft.fftshift(fft.fft((y_wn[:, 0]), axis=0)) - np.squeeze(y_out_mean)),
            0,
            5,
            True,
            linewidth=1,
        ...
        # self.fillppm(30-2*np.std(y_out_f, 0).T, 30+2*np.std(y_out_f, 0).T, 0, 5, True, alpha=.1, color='red') # noqa: E501
    ...
    def monteCarlo(self, n, f, d, nl, ph, load):
        ...
        if self.parameters["test_load"]:
            ...
            noise = np.random.normal(0, nl, (n, self.sigLen)) + 1j * np.random.normal(
                0, nl, (n, self.sigLen)
            )
            y_test = y_wn + noise.T
            # min_c, max_c, f, d, ph, noiseLevel, ns, mm_cond
        ...
        plot = sns.jointplot(
            ...
            kind="kde",
            cmap=cmap,
            marginal_kws={"shade": "True", "alpha": 0.2},
            shade=True,
            shade_lowest=False,
            alpha=0.5,
        )
        ...
        g = sns.scatterplot(
            y="variable",
            x="value",
            data=dfm[dfm["variable"].isin(mean)],
            hue="variable",
            alpha=0.1,
            zorder=1,
        )
    ...
    def erroVsnoise(self, n, nl, f, d, ph, load):
        ...
        if self.parameters["test_load"]:
            ...
            for i in range(0, n):
                noise = np.random.normal(
                    0, nl_local[i], (1, self.sigLen)
                ) + 1j * np.random.normal(0, nl_local[i], (1, self.sigLen))
        ...
        errs = (
            np.abs(
                np.divide(
                    np.squeeze(np.array(df[mean])) - ampl_t[:, 0 : len(mean)],
                    ampl_t[:, 0 : len(mean)],
                )
            )
            * 100
    ...
    def predict_ensembles(self, y_test, mean_=True):
        ...
        for autoencoder in self.autoencoders:
            ...
            dect, enct, _, fr, damp, ph, _, dec = self.testmodel(
                autoencoder, y_test.T.cuda()
            )
        ...
        if self.MM_type == "single" or self.MM_type == "single_param":
            ampl = np.concatenate(
                (
                    encs_np[:, :, 0 : self.numOfSig],
                    np.expand_dims(encs_np[:, :, 1 * self.numOfSig], axis=2),
                ),
                axis=2,
            )
            if mean_:
    ...
    def quantify(self):
        ...
        #     self.plotsppm(fft.fftshift(fft.fft((y_n[0:1024, rng]), axis=0)), 0, 5, False, linewidth=0.3, linestyle='-') # noqa: E501
        #     self.plotsppm(-30 + y_out_f[0, rng, 0:1024].T, 0, 5, False, linewidth=0.3, linestyle='--') # noqa: E501
        #     self.plotsppm(+30 + (fft.fftshift(fft.fft((y_n[0:1024, rng]), axis=0)) - np.squeeze(y_out_f[0, rng,:].T)), 0, 5, True, linewidth=0.3, linestyle='-') # noqa: E501
        ...
        for ll in range(0, 5):
            ...
            self.plotppm(
                20 + np.fft.fftshift(np.fft.fft((y_test[0:1024, id])).T),
                rang[0],
                rang[1],
                False,
                linewidth=0.3,
            ...
            rec_signal, _, enc, fr, damp, ph, mm_v, _ = self.testmodel(
                self.autoencoders[0], y_test.T.cuda()
            )
            self.plotppm(
                np.fft.fftshift(
                    np.fft.fft(
                        ((mm_v).cpu().detach().numpy()[id, 0 : self.truncSigLen])
                    ).T
                ),
                rang[0],
                rang[1],
            ...
            self.plotppm(
                ...
                + np.fft.fftshift(
                    np.fft.fft(
                        (rec_signal.cpu().detach().numpy()[id, 0 : self.truncSigLen])
                    ).T
                ),
                rang[0],
                rang[1],
            ...
            self.plotppm(
                ...
                + np.fft.fftshift(
                    np.fft.fft(
                        (
                            y_test[0 : self.truncSigLen, id]
                            - rec_signal.cpu()
                            .detach()
                            .numpy()[id, 0 : self.truncSigLen]
                        )
                    ).T
                ),
                rang[0],
            ...
            sns.despine()
            self.plot_basis(
                (enc[id, :].unsqueeze(dim=0)).cpu().detach().numpy(),
                fr[id, :].cpu().detach().numpy(),
            ...
            if "param" in self.MM_type:
                ...
                for idx in range(0, self.numOfMM):
                    ...
                    if idx == self.numOfMM - 1:
                        ...
                        self.plotppm(
                            -10 * idx + fft.fftshift(fft.fft(x)).T, 0, 5, False
                        )
                self.savefig("MM")
                self.mm = mm.T
                Jmrui.write(
            ...
            for i in range(0, self.ens):
                ...
                if self.parameters["early_stop"][0]:
                    early_stopping = EarlyStopping(
                        "val_acc", patience=self.parameters["early_stop"][1]
                    )
                    trainer = pl.Trainer(
                        gpus=1,
                ...
                # trainer= pl.Trainer(gpus=1, max_epochs=self.max_epoch, logger=logger,callbacks=[lr_monitor]) # noqa: E501
                logger.save()
                device = torch.device(self.parameters["gpu"])
                temp = Encoder_Model(
                    self.depths[i], self.betas[i], self.reg_wei[i], self
                ).to(device)
                x = summary(temp.met, (2, 1024))
                trainer.fit(
                    temp,
                    DataLoader(self.train, batch_size=self.batchsize, shuffle=True),
                    DataLoader(self.val, batch_size=self.batchsize),
                )
    ...
    def dotest(self):
        ...
        for i in range(0, self.ens):
            ...
            # macs, params = profile(model.met, inputs=(torch.randn(1, 2, 1024),))                      # noqa: E501
            # macs, params = get_model_complexity_info(model.met.encoder, (2, 1024), as_strings=True,   # noqa: E501
            #                                          print_per_layer_stat=True, verbose=True)         # noqa: E501
            # print('{:<30}  {:<8}'.format('Computational complexity: ', macs))                         # noqa: E501
            # print('{:<30}  {:<8}'.format('Number of parameters: ', params))                           # noqa: E501
    ...
    def tuner(self):
        """
        The function takes in the parameters of the model and the data, and then
        uses the Ray Tune library to find the best hyperparameters for the model.

        The function is called in the main function, and the best
        hyperparameters are then used to train the model.
        ...
        scheduler = HyperBandForBOHB(
            time_attr="training_iteration", max_t=self.max_epoch, stop_last_trials=False
        )
...
def tunermodel(config, engine=None):
    ...
    if engine.parameters["early_stop"][0]:
        early_stopping = EarlyStopping(
            "val_acc", patience=engine.parameters["early_stop"][1]
        )
        trainer = pl.Trainer(
            gpus=1,
            max_epochs=engine.max_epoch,
            logger=logger,
            ...
            callbacks=[
                ...
                lr_monitor,
                TuneReportCheckpointCallback(
                    metrics={"mean_accuracy": "performance"},
                    filename="checkpoint",
                    on="validation_end",
                ),
            ]
    ...
    trainer.fit(
        temp.to(engine.parameters["gpu"]),
        DataLoader(engine.train, batch_size=config["batchsize"], num_workers=255),
