...
# The Encoder_Model class inherits from the LightningModule class, and it has a
# constructor that takes in a dictionary of hyperparameters
class Encoder_Model(pl.LightningModule):
    def __init__(self, depth, beta, tr_wei, param):
        ...
        # self.max_c = torch.from_numpy(np.array(param.max_c)).float().cuda(self.param.parameters['gpu']) # noqa: E501
        if self.param.MM_constr:
            print("tr is not in the model")
        else:
            self.tr = nn.Parameter(
                torch.tensor(0.004).cuda(self.param.parameters["gpu"]),
                requires_grad=True,
            )
        self.tr_wei = tr_wei
        ...
        if self.param.MM:
            if self.param.MM_type == "single" or self.param.MM_type == "single_param":
                ...
                self.mm = (
                    torch.from_numpy(param.mm[0:2048].astype("complex64"))
                    .cuda(self.param.parameters["gpu"])
                    .T
                )
            if self.param.MM_type == "param":
                if not self.param.MM_fd_constr:
                    self.enc_out = (
                        (1 * (param.numOfSig) + 3 * 1) + (self.param.numOfMM * 2) + 1
                    )
                else:
        ...
        #         self.mm = torch.from_numpy(param.mm[0:2048].astype('complex64')).cuda().T # noqa: E501
        #     if self.param.MM_type == 'param':
        ...
        if param.enc_type == "convnext_tiny":
            ...
            self.met = MLPMixer1d(
                in_channels=2,
                image_size=self.param.truncSigLen,
                patch_size=16,
                z_dim=self.enc_out,
                dim=512,
                depth=8,
            ...
            self.met = self.model(
                depth, param.banorm, self.enc_out, self.param.parameters["kw"]
            )
    ...
    def sign(self, t, eps):
        """
        sgn function
        It takes in a tensor t and a small number eps, and returns a tensor of the
        same shape as t
    ...
    def Gauss(self, ampl, f, d, ph, Crfr, Cra, Crd):
        """
        It returns a complex-valued Gaussian function with amplitude, frequency, decay,
        phase, frequency reference, amplitude reference, and decay reference parameters
        ...
        """
        return (Cra * ampl) * torch.multiply(
            torch.multiply(
                torch.exp(ph * 1j),
    ...
    def Lornz(self, ampl, f, d, ph, Crfr, Cra, Crd):
        """
        It returns a complex exponential with a frequency of $f + Crfr$, a phase of
        $ph$, an amplitude of $Cra*ampl$, and a decay of $d+Crd$
    ...
    def Voigt(self, ampl, f, dl, dg, ph, Crfr, Cra, Crd):
        """
        The function takes in a set of parameters and returns a complex valued Voigt
        function
    ...
    def forward(self, x):
        """
        The function takes in a signal, passes it through a neural network, and returns
        the signal that the model-decoder predicts

        :param x: input signal
        :return: The output of the forward function is the reconstructed signal, the
        encoded signal, the encoded signal without the MM, the frequency, the damping,
        the phase, the reconstructed MM signal, and the reconstructed signal without
        the MM.
        """
        mm_rec = 0
        ...
        # enc = self.sigm(enc)
        # fr = self.reparameterize(torch.unsqueeze(enct[:, -5],1),torch.unsqueeze(enct[:, -6],1))   # noqa: E501
        # damp = self.reparameterize(torch.unsqueeze(enct[:, -3],1),torch.unsqueeze(enct[:, -4],1)) # noqa: E501
        # ph = self.reparameterize(torch.unsqueeze(enct[:, -1],1),torch.unsqueeze(enct[:, -2],1))   # noqa: E501
        ...
        if self.param.MM:
            if self.param.MM_type == "single" or self.param.MM_type == "single_param":
                ...
                mm_rec = torch.multiply(
                    mm_rec, torch.exp(-2 * math.pi * (fr) * self.t.T * 1j)
                )
            ...
            if self.param.MM_type == "param":
                if not self.param.MM_fd_constr:
                    ...
                    for idx in range(0, len(self.param.MM_f)):
                        mm_rec += self.MM_model(
                            (mm_enc[:, idx].unsqueeze(1)),
                            torch.unsqueeze(mm_f[:, idx], 1),
                            torch.unsqueeze(mm_damp, 1),
                            torch.tensor(0),
                            self.param.trnfreq * (self.param.MM_f[idx]),
                            self.param.MM_a[idx],
                            self.param.MM_d[idx],
                        )
                else:
                    ...
                    #                          mm_enct[:, (self.param.numOfMM):(self.param.numOfMM) * 2]) # noqa: E501
                    for idx in range(0, len(self.param.MM_f)):
                        mm_rec += self.MM_model(
                            (mm_enc[:, idx].unsqueeze(1)),
                            fr,
    ...
    def reparameterize(self, mu: Tensor, logvar: Tensor) -> Tensor:
        ...
        The reparameterization trick is a way to sample from a Gaussian distribution
        without having to explicitly sample from a Gaussian distribution.
    ...
    def loss_function(self, *args, **kwargs) -> dict:
        ...
        loss_real = 0
        loss_imag = 0
        if self.param.MM and self.param.MM_constr:
            cut_signal = ((1 + self.sign(self.t - self.tr, 0.0000001)) / 2)[
                0 : self.param.truncSigLen
            ].T * input[:, 0 : self.param.truncSigLen]
            cut_dec = (
                (1 + self.sign(self.t - self.tr, 0.0000001)) / 2
            ).T * recons.clone()
            loss_real = self.criterion(
                cut_dec.real[:, 0 : self.param.truncSigLen],
                cut_signal.real[:, 0 : self.param.truncSigLen],
            )
            loss_imag = self.criterion(
                cut_dec.imag[:, 0 : self.param.truncSigLen],
            ...
            met_loss = (
                met_loss + (self.tr_) * self.tr_wei * (self.param.batchsize)
            ) * 0.5
        ...
        init_point = 1
        loss_real = self.criterion(
            recons.real[:, init_point : self.param.truncSigLen],
            input.real[:, init_point : self.param.truncSigLen],
        )
        loss_imag = self.criterion(
            recons.imag[:, init_point : self.param.truncSigLen],
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_mse, recons_loss = [
            lo / len(x)
            for lo in self.loss_function(
                dec, x, enct[:, 0 : (self.param.numOfSig)], _, _, batch_idx, mm
            )
        ]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if (
            self.current_epoch % self.param.parameters["val_freq"] == 0
            and batch_idx == 0
        ):
            ...
            # # plt.title(str(r2))
            # plt.show()
            # ampl_t = min_c + np.multiply(np.random.random(size=(1, 21)), (max_c - max_c)) # noqa: E501
            # y_n, y_wn = getSignal(ampl_t, 0, 5, 0, 0.5)
            rang = [0, 5]
            # id= 10
            # plotppm(np.fft.fftshift(np.fft.fft((y_n.T)).T), 0, 5,False, linewidth=0.3, linestyle='-') # noqa: E501
            self.param.plotppm(
                np.fft.fftshift(np.fft.fft((self.param.y_test_trun[id, :])).T),
                rang[0],
                rang[1],
                False,
                linewidth=0.3,
            ...
            # plt.plot(np.fft.fftshift(np.fft.fft(np.conj(y_trun[id,:])).T)[250:450], linewidth=0.3) # noqa: E501
            rec_signal, _, enc, fr, damp, ph, mm_v, _ = self(
                torch.unsqueeze(self.param.y_test_trun[id, :], 0).cuda()
            )
            # plotppm(np.fft.fftshift(np.fft.fft(((rec_signal).cpu().detach().numpy()[0,0:truncSigLen])).T), 0, 5,False, linewidth=1, linestyle='--') # noqa: E501
            if self.param.MM:
                self.param.plotppm(
                    np.fft.fftshift(
                        np.fft.fft(
                            (
                                ...
                                .cpu()
                                .detach()
                                .numpy()[0, 0 : self.param.truncSigLen]
                            )
                        ).T
                    ),
                    rang[0],
            ...
            sns.despine()
            self.param.plot_basis(
                (enc).cpu().detach().numpy(),
                fr.cpu().detach().numpy(),
                damp.cpu().detach().numpy(),
                ph.cpu().detach().numpy(),
            )
            # plt.plot(np.fft.fftshift(np.fft.fft(np.conj(rec_signal.cpu().detach().numpy()[0,0:trunc])).T)[250:450], linewidth=1,linestyle='--') # noqa: E501
            plt.title("#Epoch: " + str(self.current_epoch))
            self.param.savefig(
                self.param.epoch_dir
                + "paper1_1_epoch_"
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = {
            # 'scheduler': torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min',patience=self.param.parameters['reduce_lr'][0]), # noqa: E501
            # 'scheduler': torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, 5), # noqa: E501
            # 'scheduler': torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.99), # noqa: E501
            "scheduler": torch.optim.lr_scheduler.MultiStepLR(
                optimizer,
                milestones=[
                    int(self.param.max_epoch / 8),
                    int(self.param.max_epoch / 2),
                ],
    ...
    def validation_epoch_end(self, validation_step_outputs):
        """
        It takes the output of the validation step, which is a list of lists,
        and calculates the mean of each list, which is the r2 for each metabolite.
